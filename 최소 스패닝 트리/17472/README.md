...
문제에서는 섬들간에 다리를 연결하여 모든 섬들을 이어주기를 원하고, 이 다리의 최소 길이를 요구하고 있다. 섬들을 하나의 정점으로 보고 다리를 간선으로 본다고 하면 결국 그래프 이어주기 문제가 된다. 또한 길이를 weight로 본다고 하면 간선의 weight의 합이 최소인 것을 요구하므로 MST(최소 신장 트리)를 만들 것을 요구한다고 생각 했다.
...
```
크루스칼 알고리즘을 통해서 최소 스패팅 트리 만들기를 진행 해주면 된다.  
